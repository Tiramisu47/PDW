...
function handleLogin(args) {
  ...
  W_tryLoginSession_S(sessionToken);
}

function handleToggle(args) {
  //PROTECTED
  if (!_sessionToken) {
    printLine("You need to log in first", "red");
    return;
  }
...
function handleRange(args) {
  ...
  const uid = args[1];
  const value = args[2];
  W_tryRangeElementState_S(uid, value);
}
...
function W_tryRangeElementState_S(uid, value) {
  const message = {
    channelType: "core",
    eventType: "W_tryRangeElementState_S",
    sessionToken: _sessionToken,
    elementUid: uid,
    value: value,
  };
